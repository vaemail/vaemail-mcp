...
export class ClientVaEmail {
  ...
  // --- API Brevo-compatible (/v3) : campagnes, listes, contacts. C'est elle que
  // parlent les quatre outils d'intention (lib/intentions.js).

  listes() {
    return this.appeler('GET', '/v3/contacts/lists', { requete: { limit: 50 } });
  }

  liste(id) {
    return this.appeler('GET', `/v3/contacts/lists/${encodeURIComponent(id)}`);
  }

  creerListe(nom, dossierId) {
    const corps = { name: nom };
    if (dossierId) corps.folderId = dossierId;

    return this.appeler('POST', '/v3/contacts/lists', { corps });
  }

  /** Les dossiers n'existent pas sur toutes les versions du service : null si la route manque. */
  async dossiers() {
    try {
      return await this.appeler('GET', '/v3/contacts/folders', { requete: { limit: 50 } });
    } catch (e) {
      if (e instanceof ErreurVaEmail && e.statut === 404) return null;
      throw e;
    }
  }

  contact(email) {
    return this.appeler('GET', `/v3/contacts/${encodeURIComponent(email)}`);
  }

  importerContacts(lignes, listeIds) {
    // Jamais `emailBlacklist` ici : sans ce champ, un contact déjà désinscrit
    // garde son statut. Le passer réabonnerait des gens qui sont partis.
    return this.appeler('POST', '/v3/contacts/import', {
      corps: { jsonBody: lignes, listIds: listeIds, updateExistingContacts: true },
    });
  }

  campagnes(filtres) {
    return this.appeler('GET', '/v3/emailCampaigns', { requete: { limit: 200, ...filtres } });
  }

  campagne(id) {
    return this.appeler('GET', `/v3/emailCampaigns/${encodeURIComponent(id)}`);
  }

  creerCampagne(corps) {
    return this.appeler('POST', '/v3/emailCampaigns', { corps });
  }

  testerCampagne(id, emails) {
    return this.appeler('POST', `/v3/emailCampaigns/${encodeURIComponent(id)}/sendTest`, { corps: { emailTo: emails } });
  }

  programmerCampagne(id, quandIso) {
    return this.appeler('POST', `/v3/emailCampaigns/${encodeURIComponent(id)}/schedule`, { corps: { scheduledAt: quandIso } });
  }

  envoyerCampagneMaintenant(id, destinatairesAttendus) {
    return this.appeler('POST', `/v3/emailCampaigns/${encodeURIComponent(id)}/sendNow`, {
      corps: { confirm: true, expectedRecipients: destinatairesAttendus },
    });
  }

  statistiquesCampagne(id) {
    return this.appeler('GET', `/v3/emailCampaigns/${encodeURIComponent(id)}/statistics`);
  }
}
